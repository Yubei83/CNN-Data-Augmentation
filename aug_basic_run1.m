...
fprintf('\nValidation Accuracy (Aug Basic Run1): %.2f%%\n', accuracy * 100);

fid = fopen('aug_basic_run1_accuracy.txt', 'w');
fprintf(fid, 'Validation Accuracy: %.2f%%\n', accuracy * 100);
fclose(fid);

save('aug_basic_run1_model.mat', 'trainedNet');
...
title('Training Progress: Aug Basic Run1');
grid on;
saveas(f, 'aug_basic_run1_training_plot.png');
close(f);
