...
load('aug_basic_run2_model.mat', 'trainedNet');

predLabels = classify(trainedNet, augVal);
trueLabels = imdsVal.Labels;

accuracy = mean(predLabels == trueLabels);
fprintf('\nRobustness Test Accuracy (Aug Basic Run2): %.2f%%\n', accuracy * 100);

fid = fopen('aug_basic_robustness_strong_run2_accuracy.txt', 'w');
fprintf(fid, 'Robustness Test Accuracy (Aug Basic Run2): %.2f%%\n', accuracy * 100);
fclose(fid);