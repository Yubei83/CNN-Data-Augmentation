...
fprintf('\nValidation Accuracy (Aug Basic Run2): %.2f%%\n', accuracy * 100);

fid = fopen('aug_basic_run2_accuracy.txt', 'w');
fprintf(fid, 'Validation Accuracy: %.2f%%\n', accuracy * 100);
fclose(fid);

save('aug_basic_run2_model.mat', 'trainedNet');
...
title('Training Progress: Aug Basic Run2');
grid on;
saveas(f, 'aug_basic_run2_training_plot.png');
close(f);
