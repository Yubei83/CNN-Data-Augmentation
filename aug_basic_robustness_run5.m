...
load('aug_basic_run5_model.mat', 'trainedNet');

predLabels = classify(trainedNet, augVal);
trueLabels = imdsVal.Labels;

accuracy = mean(predLabels == trueLabels);
fprintf('\nRobustness Test Accuracy (Aug Basic Run5): %.2f%%\n', accuracy * 100);

fid = fopen('aug_basic_robustness_strong_run5_accuracy.txt', 'w');
fprintf(fid, 'Robustness Test Accuracy (Aug Basic Run5): %.2f%%\n', accuracy * 100);
fclose(fid);