...
fprintf('\nValidation Accuracy (Selective Aug Run1): %.2f%%\n', accuracy * 100);

% Save accuracy log
fid = fopen('selective_aug_run1_accuracy.txt', 'w');
fprintf(fid, 'Validation Accuracy: %.2f%%\n', accuracy * 100);
fclose(fid);

% Save model
save('selective_aug_run1_model.mat', 'trainedNet');
...
title('Training Progress: Selective Aug Run1');
grid on;
saveas(f, 'selective_aug_run1_training_plot.png');
close(f);
