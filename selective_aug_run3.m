...
fprintf('\nValidation Accuracy (Selective Aug Run3): %.2f%%\n', accuracy * 100);

% Save accuracy log
fid = fopen('selective_aug_run3_accuracy.txt', 'w');
fprintf(fid, 'Validation Accuracy: %.2f%%\n', accuracy * 100);
fclose(fid);

% Save model
save('selective_aug_run3_model.mat', 'trainedNet');
...
title('Training Progress: Selective Aug Run3');
grid on;
saveas(f, 'selective_aug_run3_training_plot.png');
close(f);