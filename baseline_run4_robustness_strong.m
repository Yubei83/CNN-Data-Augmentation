...
load('run4_model.mat', 'trainedNet');

predLabels = classify(trainedNet, augVal);
trueLabels = imdsVal.Labels;

accuracy = mean(predLabels == trueLabels);
fprintf('\nRobustness Test Accuracy (Baseline Run4): %.2f%%\n', accuracy * 100);

fid = fopen('baseline_run4_robustness_strong_accuracy.txt', 'w');
fprintf(fid, 'Robustness Test Accuracy (Baseline Run4): %.2f%%\n', accuracy * 100);
fclose(fid);