...
load('aug_adv_run4_model.mat', 'trainedNet');

predLabels = classify(trainedNet, augVal);
trueLabels = imdsVal.Labels;

accuracy = mean(predLabels == trueLabels);
fprintf('\nRobustness Test Accuracy (Aug Advanced Run4): %.2f%%\n', accuracy * 100);

fid = fopen('aug_adv_robustness_run4_strong_accuracy.txt', 'w');
fprintf(fid, 'Robustness Test Accuracy (Aug Advanced Run4): %.2f%%\n', accuracy * 100);
fclose(fid);