...
fprintf('\nValidation Accuracy (Aug Basic Run4): %.2f%%\n', accuracy * 100);

fid = fopen('aug_basic_run4_accuracy.txt', 'w');
fprintf(fid, 'Validation Accuracy: %.2f%%\n', accuracy * 100);
fclose(fid);

save('aug_basic_run4_model.mat', 'trainedNet');
...
title('Training Progress: Aug Basic Run4');
grid on;
saveas(f, 'aug_basic_run4_training_plot.png');
close(f);
